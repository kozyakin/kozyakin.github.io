...
% arara: bibtexu: {options: ['-H', '-l', 'ru', '-o', 'ru']}
...
\sloppy Style files \texttt{amsbib.bst} and \texttt{amsbibs.bst}, and example files \texttt{example.tex} and \texttt{\detokenize{example_en.tex}} can be downloaded from \href{https://github.com/kozyakin/bibtex_to_amsbib}{BibTeX to AMSBIB} of my GitHup~Pages repository. The files of the AMSBIB package (\texttt{amsbib.sty} + \texttt{*.pdf}) required for translating examples are borrowed from 
