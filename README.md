...
Демонстрация применения преобразования библиографии из формата BibTeX в формат AMSBIB демонстрируется в примере `example.tex`.*
...
To simplify and unify the process of converting bibliography from the BibTeX format to the AMSBIB format, the style files `amsbib.bst` and `amsbibs.bst` are presented here, which perform such conversion automatically. Moreover, the first of these style files creates a list of AMSBIB bibliographic records in the order of citation of publications in the work, and the second one in alphabetical order.

A demonstration of applying the transformation of bibliography from BibTeX format to AMSBIB format is shown in the example `example_en.tex`.*
